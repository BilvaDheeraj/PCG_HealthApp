...
import { Bell, AlertTriangle, CheckCircle, Calendar, TrendingUp, Info, Trash2, Loader2, Link, ShieldAlert } from "lucide-react";
...
const typeIcons: Record<string, JSX.Element> = {
  ...
  security: <ShieldAlert className="w-5 h-5 text-admin" />,
  system: <Link className="w-5 h-5 text-health-blue" />,
};
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <motion.div className="flex items-center justify-between" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div>
            <h1 className="text-2xl font-display font-bold text-foreground flex items-center gap-2">
              <Bell className="w-6 h-6 text-admin" /> Notifications
            </h1>
            <p className="text-muted-foreground text-sm mt-1">{unreadCount} unread system notifications</p>
          </div>
          {unreadCount > 0 && (
            <button onClick={markAllRead} className="text-xs text-admin hover:underline">Mark all as read</button>
          )}
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            <motion.div
              key={notif.id}
              className={`glass-card p-5 flex items-start gap-4 transition-all ${!notif.read ? "border-admin/20 bg-admin/5" : "opacity-70"}`}
            ...
            >
              ...
              {!notif.read && <div className="w-2 h-2 rounded-full bg-admin flex-shrink-0 mt-2" />}
            </motion.div>
          ))}
        </div>
        )}
