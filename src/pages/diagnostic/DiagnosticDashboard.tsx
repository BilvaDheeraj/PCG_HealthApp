...
const DiagnosticDashboard = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/api/diagnostic/dashboard")
      .then(res => res.json())
      .then(fetchedData => {
        setData(fetchedData);
        setLoading(false);
  ...
  const stats = data?.stats || { todays_bookings: 0, pending_reports: 0, patients_served: 0, todays_revenue: 0 };
  const bookings = data?.bookings || [];
  const testDistribution = data?.testDistribution || [];
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            value={stats.patients_served.toString()}
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie
                  ...
                  stroke="none"
                >
                  {testDistribution.map((entry: any, i: number) => (
